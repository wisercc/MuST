...
import logging
from utils import SeizureDataset
import random
import numpy as np
...
Net = 'EEGSwin'
sub = 1
logging.basicConfig(filename='results/Epilepsy/' + Net + '/training_log_' + '.log', level=logging.INFO,
...
weight_decay = 0
num_epochs = 2000
warmup_epochs = 200  # Warm-up阶段的epoch数
n_classes = 4
...
# model = EEGNet(nb_classes=2, Chans=1, Samples=4096).to(device)
# model = Conformer().to(device)
# model = SwinTransformer1D(num_classes=2,in_chans=1,embed_dim=40,depths=[2,2,2],num_heads=[2,4,8],window_size=5,mlp_ratio=4).to(device)
model = Must(n_classes=2,emb_size=100).to(device)
# model = EEGViT(num_chan=1, num_time=4096, num_patches=128, num_classes=2).to(device)
# model = Deformer(num_chan=1, num_time=4096, temporal_kernel=11, num_kernel=40,
#                  num_classes=2, depth=4, heads=10,
