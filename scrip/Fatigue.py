...
import logging
from utils import FatigDataset
import random
import numpy as np
...
logging.basicConfig(filename='results/Fatigue/'+Net+'/training_log_' + str(sub) + '.log', level=logging.INFO,
...
# model = EEGNet(nb_classes=4,Chans=30,Samples=384).to(device)
# model = Conformer().to(device)
# model = SwinTransformer1D(num_classes=5,in_chans=30,embed_dim=40,depths=[2,2,2],num_heads=[2,4,8],window_size=5,mlp_ratio=4).to(device)
# model = EEGViT(num_chan=30, num_time=384, num_patches=16, num_classes=4).to(device)
# model = Deformer(num_chan=30, num_time=384, temporal_kernel=11, num_kernel=40,
#                  num_classes=4, depth=4, heads=10,
